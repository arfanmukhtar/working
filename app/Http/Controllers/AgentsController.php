...
class AgentsController extends Controller
{
    ...
    {
		$agents = Agent::get();
        return view('agents/home' , ['agents' => $agents, 'title' => "Agents"]);
    }
	...
	public function save(Request $request)
    {
		...
		if(!empty(Input::file('photo')) and Input::file('photo')->isValid()) {
            ...
            Input::file('photo')->move($destinationPath, "profile.jpg");
            $path = public_path("assets/images/uploads/$id/temp/profile.jpg"); 
	...
	public function verify_email()
	 {
            ...
            Mail::send('emails.activation', $data_Send, function($message) use ($data_Send) {
	 ...
	 public function update_profile(Request $request) {
		...
		if(!empty(Input::file('photo')) and Input::file('photo')->isValid()) {
            $destinationPath = "assets/images/uploads/$id/"; 
            Input::file('photo')->move($destinationPath, "profile.jpg");
            $path = public_path("assets/images/uploads/$id/profile.jpg"); 
            Image::make($path)->resize(150, null, function ($constraint) {
				$constraint->aspectRatio();
			})->save($path);
        }
	 ...
	 public function password_reset(Request $request) { 
			...
			if(Hash::check($old_password, $user->password))
			{
				...
				DB::table("users")->where("ID_User" , Auth::user()->ID_User)->update($data);
